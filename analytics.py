...
def show_analytics_dashboard(hosts_list: list) -> None:
    """Calculates vector metrics using NumPy and renders a graphical report window."""
    if not hosts_list:
        messagebox.showinfo("Dashboard Info", "Database is empty. Please add network assets first.")
        return

    # Extract raw data vectors for NumPy computations
    ...
    # Core NumPy statistical operations
    avg_open_ports = np.mean(total_ports_array)
    max_open_ports = np.max(total_ports_array)

    # Risk categorization counts via array masking assertions
    ...
    # Build Pop-up GUI Dashboard Window
